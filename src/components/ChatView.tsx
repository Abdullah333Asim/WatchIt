...
import { Send, Sparkles, PlusCircle, ArrowUp, Menu, Plus, MessageSquare, X, Film } from "lucide-react";
...
function RecommendationCard({ rec, onAddWatchlist, isAdded }: { rec: any, onAddWatchlist: (movieId: string) => void, isAdded: boolean }) {
  const [imgError, setImgError] = useState(false);
  const showImage = rec.poster_url && !imgError;

  return (
    <div className="bg-black/40 border border-white/10 rounded-xl overflow-hidden flex flex-col shadow-lg">
      {showImage ? (
        <div className="h-48 w-full shrink-0 relative bg-[#1c1b1b]">
          <img 
            src={rec.poster_url} 
            alt={rec.title ? `Poster for ${rec.title}` : "Movie poster"} 
        ...
        <div className="h-24 w-full shrink-0 relative bg-gradient-to-r from-[#222] via-[#191919] to-[#222] flex items-center justify-between px-4 border-b border-white/5">
      ...
      <div className={`p-4 flex flex-col flex-1 gap-2 relative z-10 ${showImage ? '-mt-10' : ''}`}>
        ...
        <div className="mt-auto pt-4 flex flex-col gap-3">
          ...
          {rec.movie_id && (
            <button 
              onClick={() => onAddWatchlist(rec.movie_id)}
              disabled={isAdded}
            ...
            >
              {isAdded ? <><Sparkles className="w-3 h-3" /> Added to Watchlist</> : <><Plus className="w-3 h-3" /> Add to Watchlist</>}
            </button>
          )}
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-2">
            {data.recommendations.map((rec: any, idx: number) => (
              <RecommendationCard 
                ...
                isAdded={addedIds.has(rec.movie_id)} 
              />
            ))}
          </div>
        </div>
