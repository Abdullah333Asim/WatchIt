       ...
       for (const m of data.results) {
         ...
         try {
           ...
           if (detailRes.ok) {
             ...
             if (detailData.runtime) duration = `${detailData.runtime}m`;
           }
         } catch(e) {}
         ...
         if (process.env.DATABASE_URL) {
           try {
             await db.insert(movies).values({
               id: m.id.toString(), title: m.title, year, genre: genres || 'Unknown', duration: duration, synopsis: m.overview || 'No synopsis available.', posterUrl: `https://image.tmdb.org/t/p/w500${m.poster_path}`, rating: m.vote_average ? parseFloat(m.vote_average.toFixed(1)) : 0
             }).onConflictDoNothing();
           } catch (e) {}
         }
       }
  ...
  try {
    ...
    if (data.results && data.results.length > 0) {
      ...
      if (process.env.DATABASE_URL) {
        try {
          await db.insert(movies).values({
            id: m.id.toString(), title: m.title, year, genre: genres || 'Unknown', duration, synopsis: m.overview || 'No synopsis available.', posterUrl: poster_url, rating: m.vote_average ? parseFloat(m.vote_average.toFixed(1)) : 0
          }).onConflictDoUpdate({
            target: movies.id,
            set: { posterUrl: poster_url, title: m.title, year, rating: m.vote_average ? parseFloat(m.vote_average.toFixed(1)) : 0 }
          });
        } catch (e) {}
      }
      
      return { id: m.id.toString(), title: m.title, year, poster_url, rating: m.vote_average ? parseFloat(m.vote_average.toFixed(1)) : 0 };
    }
  } catch (e) {
